...
export async function generateMetadata(props: PageProps): Promise<Metadata> {
  ...
  const service = await getServiceById("shraddha", params.id);

  if (!service) {
    return { title: "श्राद्ध विधी मिळाली नाही" };
  }
...
export default async function ShraddhDetailPage(props: PageProps) {
  const params = await props.params;
  const service = await getServiceById("shraddha", params.id);
        ...
        <div className="container-custom section-padding">
          ...
          {service.benefits && (
            <div className="mt-16 space-y-6">
              <div>
                <h2 className="text-2xl font-bold text-red-950">महत्व</h2>
                <div className="gold-line !mx-0 !mt-3" />
                <div className="mt-4 whitespace-pre-wrap text-amber-950/70">
                  {service.benefits}
                </div>
